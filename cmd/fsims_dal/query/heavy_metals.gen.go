...
func newHeavyMetal(db *gorm.DB, opts ...gen.DOOption) heavyMetal {
	...
	_heavyMetal.TimeRecordAt = field.NewString(tableName, "time_record_at")
	_heavyMetal.As = heavyMetalHasOneAs{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("As", "pasture.As"),
	}
...
func (h *heavyMetal) updateTableName(table string) *heavyMetal {
	...
	h.TimeRecordAt = field.NewString(table, "time_record_at")
...
type heavyMetalHasOneAs struct {
	db *gorm.DB

	field.RelationField
}

func (a heavyMetalHasOneAs) Where(conds ...field.Expr) *heavyMetalHasOneAs {
	if len(conds) == 0 {
		return &a
	}
...
func (a heavyMetalHasOneAs) WithContext(ctx context.Context) *heavyMetalHasOneAs {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a heavyMetalHasOneAs) Session(session *gorm.Session) *heavyMetalHasOneAs {
	a.db = a.db.Session(session)
	return &a
}

func (a heavyMetalHasOneAs) Model(m *pasture.HeavyMetal) *heavyMetalHasOneAsTx {
	return &heavyMetalHasOneAsTx{a.db.Model(m).Association(a.Name())}
}

type heavyMetalHasOneAsTx struct{ tx *gorm.Association }

func (a heavyMetalHasOneAsTx) Find() (result *pasture.As, err error) {
	return result, a.tx.Find(&result)
}

func (a heavyMetalHasOneAsTx) Append(values ...*pasture.As) (err error) {
...
func (a heavyMetalHasOneAsTx) Replace(values ...*pasture.As) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a heavyMetalHasOneAsTx) Delete(values ...*pasture.As) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a heavyMetalHasOneAsTx) Clear() error {
	return a.tx.Clear()
}

func (a heavyMetalHasOneAsTx) Count() int64 {
	return a.tx.Count()
}
